            ...
            if wizard.advance_payment_method == 'percentage':
                for order_id in wizard._context.get('active_ids', []):
                    ...
                    for invoice in sale_order.invoice_ids:
                        ...
                        if not dp_lines:
                            continue
                        # جمع كل القيم اللي في الداون بايمنت
                        total_dp = sum(dp_lines.mapped('price_subtotal'))
                        # امسح كل سطور الداون بايمنت
                        invoice.write({'invoice_line_ids': [(2, line.id) for line in dp_lines]})
                        # أضف واحد جديد بالسالب (وبدون ضرائب)
                        invoice.write({'invoice_line_ids': [(0, 0, {
                            'product_id': self.env.ref('sale.advance_product_0').id,
                            'quantity': 1,
                            'price_unit': -abs(total_dp),
