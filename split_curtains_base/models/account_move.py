...
import logging
from odoo.fields import Date # التأكد من استيراد Date
    ...
    def _check_and_create_purchase_orders(self):
        for inv in self.filtered(lambda m: m.move_type == 'out_invoice' and m.payment_state == 'paid'):
            ...
            for sl in sale_order.order_line:
                ...
                # التأكد من أن المنتج ليس خدمة وليس دفعة مقدمة
                if sl.product_id and sl.product_id.type != 'service' and not is_downpayment_product:
                    try:
                        # Odoo's standard method to launch stock rules (create PO/MO)
                        moves = sl._action_launch_stock_rule()
                        for move in moves:
                            if move.purchase_line_id and move.purchase_line_id.order_id:
                                # ضبط تاريخ الاستلام المخطط ليكون اليوم (حل مشكلة Yesterday)
                                move.purchase_line_id.order_id.date_planned = Date.today()
            ...
            if created_purchase_orders:
                ...
                for po in created_purchase_orders:
                    # تأكيد الـ POs إذا كانت في حالة Draft (مهم جدًا)
                    if po.state == 'draft':
                        po.button_confirm() 
                        _logger.info(f"Confirmed PO: {po.name}")
                    # ضبط عملة أمر الشراء لتكون عملة الشركة (جنيه مصري) إذا كانت مختلفة
                    if po.currency_id != self.env.company.currency_id:
                        po.currency_id = self.env.company.currency_id.id
                        _logger.info(f"PO {po.name} currency set to company currency: {po.currency_id.name}")
